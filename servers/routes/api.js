...
router.post('/api/update', async(ctx, next) => {
    ...
    let upt = await mysqljs.findDataArtice(img_name).then(result => { //查询文件明是否存在
    ...
    // let photoImgPath = path.join(__dirname,`../public/img/Photo/${name}`);
    // let fileImgPath = path.join(__dirname,`../public/img/file/${name}`);
})
// 查询所有文章图片
...
router.post('/api/deleteArticleImg', async(ctx, next) => {
    ...
    await  mysqljs.commonality(`delete from articleImg where img_name='${ctx.request.body.img_name}';`).then(result => {
       // const img_name = ctx.request.body.img_name
       // const _sql = `delete from articleImg where img_name='${img_name}';`
 
            fs.unlink(path.join(__dirname, `../public/img/artImg/${ctx.request.body.img_name}`), (err) => {
                if (!err) {
                     ctx.body = {
                        success: true,
                        data: {},
            ...
            // let paths = path.join(__dirname, `../public/img/artImg/${img_name}`);
